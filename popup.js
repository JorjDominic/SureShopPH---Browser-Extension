...
function performScan(isAutomatic = false, withReviews = false) {
  chrome.storage.local.get("accessToken", ({ accessToken }) => {
    ...
    scanBtn.innerHTML = isAutomatic ? '<i class="fas fa-sync spinning"></i> Auto-scanning...' : '<i class="fas fa-sync spinning"></i> Scanning...';
    if (withReviews) {
              ...
              if (withReviews) {
                ...
                if (isShopee) {
                  chrome.tabs.sendMessage(currentTab.id, { type: "EXTRACT_REVIEWS" }, (reviewResponse) => {
                    appendReviewsToOutput(reviewResponse?.reviews || []);
                  });
                }
              }
            } else {
...
function appendReviewsToOutput(reviews) {
  const stars = (n) => n ? "★".repeat(Math.min(n, 5)) + "☆".repeat(Math.max(0, 5 - Math.min(n, 5))) : "—";
  ...
  if (reviews.length === 0) {
    divider.innerHTML = `
      <div class="reviews-container">
        ...
        <div class="no-reviews-msg">No comments found.<br>Scroll down to the reviews section first, then scan again.</div>
      </div>`;
  } else {
    const cards = reviews.map(r => `
      <div class="review-card">
        <div class="review-card-top">
          ...
          <span class="review-stars">${stars(r.rating_stars)}</span>
        </div>
...
function resetCommentsButton() {
  commentsBtn.innerHTML = '<i class="fas fa-comments"></i> Comments Scan';
  commentsBtn.disabled = false;
}
